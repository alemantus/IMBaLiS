...
void loop() {
  ...
  */
  for (uint16_t i = 0; i < samples; i++) {
    if (count == 1) {
      ...
      vReal[i] = data2[i]; // import from data
    } else if (count == 3) {
      vReal[i] = data3[i]; // import from data
    } else if (count == 4) {
      vReal[i] = data4[i]; // import from data
    }
  ...
  */
  ...
  if (count == 1) {
    LP_filter(vReal, 256, samplingFrequency, samplingFrequency/6);
    for (uint16_t i = 0; i < 128; i++) {
      data_half[i] = vReal[i * 2];
    }
    count++;
  } else if (count == 2) {
    LP_filter(vReal, 256, samplingFrequency, samplingFrequency/6);
    for (uint16_t i = 0; i < 128; i++) {
      data_half[i + 128] = vReal[i * 2];
    }
    LP_filter(data_half, 256, samplingFrequency/2, samplingFrequency/12);
    for (uint16_t i = 0; i < 128; i++) {
      data_quarter[i] = data_half[i * 2];
    }
    count++;
  } else if (count == 3) {
    LP_filter(vReal, 256, samplingFrequency, samplingFrequency/6);
    ...
    startTime = micros();
    Approx_FFT(vReal, 256, samplingFrequency, fft_result);   //to get top five value of frequencies of X having 64 sample at 100Hz sampling
    stopTime = micros();
    if (debugMessages) {
    ...
    for (uint16_t i = 64; i < 128; i++) {
      outbuffer[i + 256 - 64 - 64] = fft_result[i];
      //Serial.println(fft_result[i]);
    }
    
    startTime = micros();
    Approx_FFT(data_half, 256, samplingFrequency, fft_result);   //to get top five value of frequencies of X having 64 sample at 100Hz sampling
    stopTime = micros();
    if (debugMessages) {
      totalTimeEasyFFT = stopTime - startTime;
      Serial.print("approxFFT: Total time is [uS] ");  // debug printing
    ...
    LP_filter(data_half, 256, samplingFrequency/2, samplingFrequency/12);
    for (uint16_t i = 0; i < 128; i++) {
      data_quarter[i + 128] = data_half[i * 2];
    }
    
    for (uint16_t i = 64; i < 128; i++) {
      outbuffer[i + 256 - 128 - 64] = fft_result[i];
      //Serial.println(fft_result[i]);
    }
    startTime = micros();
    Approx_FFT(data_quarter, 256, samplingFrequency, fft_result);   //to get top five value of frequencies of X having 64 sample at 100Hz sampling
    stopTime = micros();
    if (debugMessages) {
      totalTimeEasyFFT = stopTime - startTime;
      Serial.print("approxFFT: Total time is [uS] ");  // debug printing
    ...
    Approx_FFT(data1, 256, samplingFrequency, fft_result);   //to get top five value of frequencies of X having 64 sample at 100Hz sampling
